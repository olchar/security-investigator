...
| **SigninLogs** | `DeviceDetail`, `LocationDetails` may be **dynamic OR string** depending on workspace (Data Lake workspaces store them as strings) | Always use `tostring(parse_json(DeviceDetail).operatingSystem)` — works for both types. Direct dot-notation `DeviceDetail.operatingSystem` fails with SemanticError when column is string type |
